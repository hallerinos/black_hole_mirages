...
nkeys = np.unique(data[data['t']==0]['E'])[:-1]
for (idn,n) in enumerate(nkeys):
    print(n)
    data_sel = data[abs(data['E']-n)<1e-4]
...
axs[0].set_xlim(-4,+10)
axs[0].set_ylim(-10,+6)
axs[0].set_xlabel('$x/r_t$')
axs[0].set_ylabel('$y/r_t$')

csvfile = 'codes/lensing_angle_high_energies_diag_chi_1_rhot_1_vF_1.csv'
data_numeric = pd.read_csv(csvfile)
ks = np.unique(data_numeric['k(0)'])
for k in ks:
    data_slice = data_numeric[data_numeric['k(0)']==k]
    Xs, Ys = np.asarray(data_slice[f'\\rho_p/\\rho_t']), np.asarray(data_slice[f'\\varphi'])
    order = np.argsort(Xs)
    Xs, Ys = Xs[order], Ys[order]
    enth = 1
    axs[1].plot(Xs[::enth], Ys[::enth], zorder=1, label=f'$k_xa={k}$')

# axs[1].plot([], [], color='black', zorder=1, label='$\\rm Eq.\\ (11)$')
# axs[1].plot([], [], linestyle='dotted', color='black', zorder=1, label='$\\rm Eq.\\ (13)$')

axs[1].set_xscale('log')
axs[1].set_yscale('log')
axs[1].set_xlim(1,1e5)
axs[1].set_ylim(1e-5,2)
axs[1].set_xlabel('$\\rho_p$')
axs[1].set_ylabel('$\\varphi$')
axs[1].text(0.92, 0.94, f"$(b)$", transform = axs[1].transAxes)
